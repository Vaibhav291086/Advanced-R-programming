...
setwd("C:/Users/salvivb/Desktop/R specialisation/Assignment/Advanced R Programming")
...
# Create lookup table for memoization. Unlike in the Fibonacci example, memoization does not make sense for individual function calls here because
# recursive factorial calls never occur more than once for a specific input value.
# Therefore, efficiency is only gained when calculating factorials for subsequent input values
...
# Check if all functions produce the same results. R's built-in function factorial() is used to compare the results
...
# Interestingly, the factorial_reduce() produces NAs fairly early because reduce() runs into an integer overflow in the lower second digits (here starting at fact(12))

# Measure performance and create output ----------------------------------------

# Use microbenchmark and purrr package to calculate performance for different input values and for ranges of input values
